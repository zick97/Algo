...
void graph(){
  ...
  double pi1, pi0 = atan(1)*4, err, e[10000], v[10000], log_v[10000], log_e[10000];
  int i=0, j;
  ...
  while (err > 1.e-5);
  for(j=0; j<i; j++){
    cout << v[j] << " ; " << std::setprecision(12) << e[j] << endl;
  }
  ...
  i++;
  TGraphErrors *gr = new TGraphErrors(i, log_v, log_e, NULL, NULL);
  gr->SetMarkerSize(1.);
  gr->SetMarkerStyle(21);
  gr->GetYaxis()->SetTitle("Error");
  gr->GetXaxis()->SetTitle("Number of intervals");
  gr->SetLineColor(7);
  gr->Draw("AP");
  gr->Draw("C");
}
